...
class PercentScheduler(hass.Hass):
    ...
    def on_then_off(self, *args, **kwargs):
        ...
        if self.get_state(self.args['device']) != 'on':
            self.log(f'Turning {self.args["device"]} on.', 'DEBUG')
            self.turn_on(self.args['device'])

        self.log(f'Scheduling turn off of {self.args["device"]} in {turn_off_in} seconds.')
        self.run_in(self.do_turn_off, turn_off_in)
        self.log(f'Scheduling turn on of {self.args["device"]} in {turn_off_in + off_for} seconds.')
