"""
Provides the `PercentSchedule` appdaemon app that will call `turn_on` and `turn_off` to
keep a device on for X percent of the time.

The configuration includes:

    `device`: the hass entity_id of the device we want to toggle on and off
    `percent: the percentage of time, expressed as a float from 0 to 1, that we want the
        device to be on.
    ...
    `min_on_seconds_state`: A hass state entity to get number of seconds the device will
        be on. If provided will override `percent`.
...
That configuration will cause `switch.basement_fan` to be turned on for 100 seconds,
then it will shut off for 300 seconds, then it will turn back on for 100 seconds,
...
def get_on_off_time(percent: float, min_on_seconds: float = 60) -> Tuple[float, float]:
    return (
        float(min_on_seconds),
...
class PercentScheduler(hass.Hass):
    ...
    def initialize(self):
        ...
        self.log(
            f"PercentScheduler running {self.device} for {self.percent * 100} "
    ...
    def track_state(
        ...
        if (entity == percent_entity) or (entity == min_on_seconds_entity):
    ...
    def on_then_off(self, *args, **kwargs):
        ...
        self.log(f"Scheduling turn off of {self.device} " f"in {turn_off_in} seconds.")
        self._timers = [self.run_in(self.do_turn_off, turn_off_in)]

        self.log(
            f"Scheduling turn on of {self.device} "
